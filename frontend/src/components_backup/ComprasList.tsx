...
export function ComprasList({ sidebarOpen }: ComprasListProps) {
  ...
  const fetchItems = useCallback(async () => {
    ...
    try {
      const now = new Date();
      const str = format(new Date(now.getFullYear(), now.getMonth(), 1), 'yyyy-MM-01');
      const data = await ShoppingService.listByMonth(str);
      setItems(data);
    } catch {
