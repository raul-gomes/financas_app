...
export function ExtratoDialog({ open, onOpenChange, onImported }: ExtratoDialogProps) {
        ...
        try {
            const checkPayload = {
                transacoes: payload.transacoes.map((t, idx) => ({
                    index: idx,
                    transaction_date: t.data.split('/').reverse().join('-'), // DD/MM/YYYY → YYYY-MM-DD
                    amount: t.valor,
                })),
            }
    ...
    const totals = sessions.reduce((acc, s) => ({
        ...
        totalIncome: acc.totalIncome + s.transactions.filter(t => t.type === 'income').reduce((sum, t) => sum + t.valor, 0),
        totalExpense: acc.totalExpense + s.transactions.filter(t => t.type === 'expense').reduce((sum, t) => sum + t.valor, 0),
                                                    ...
                                                    return (
                                                        <tr key={rIdx} className="hover:bg-muted/30">
                                                            ...
                                                            <td className={`px-3 py-2 text-xs font-medium whitespace-nowrap ${
                                                                ...
                                                                {trans.type === 'saida' ? '-' : ''}{formatCurrency(trans.valor)}
                                                            </td>
