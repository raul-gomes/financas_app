...
export function IncomeChart({ breakdown }: IncomeChartProps) {
  ...
  const incomeData = useMemo(() => {
    if (!breakdown?.subcategorias?.length) {
      return { chartData: [], total: 0, count: 0 }
    }

    const chartData = breakdown.subcategorias.map((item, idx) => ({
      category: item.subcategoria,
      value: item.total,
      color: `hsl(${120 + (idx * 60) % 240},65%,55%)`
    }))

    const total = breakdown.subcategorias.reduce((sum, item) => sum + item.total, 0)
    const count = breakdown.subcategorias.length
