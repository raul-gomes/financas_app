...
class TransacaoResponse(TransacaoBase):
    ...
    @model_validator(mode='before')
    @classmethod
    def extrair_nomes(cls, data: any) -> any:
        # Se for um objeto ORM com relacionamentos carregados
        if hasattr(data, 'categoria') and data.categoria:
            setattr(data, 'categoria_nome', data.categoria.categoria_nome)
        if hasattr(data, 'subcategoria') and data.subcategoria:
            setattr(data, 'subcategoria_nome', data.subcategoria.subcategoria_nome)
        return data
